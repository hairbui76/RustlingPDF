...
The existing browser hook sends `sortType`,
`removeCertSign`, `generateToc`, repeated `fileInput`, and `clientFileIds` without
requiring a UI change.

## Implemented Rust Behaviour

The service accepts the documented path and multipart field names. It streams uploads
...
document-level seed. Documents use PDFium's `FPDF_ImportPages`. The seed
catalog and AcroForm remain intact while later pages and their
...
an incremental PDF revision appended with bounded memory; a round-trip test
verifies that PDFium reads those bookmarks on a subsequent
...
flattening loads the final document only when PDFium reports a signature. A
fallback uses the in-memory `lopdf` implementation only when no PDFium runtime
is configured. The request limit is 2000 MiB by default and
...
service override. Large-file fidelity fixtures continue to protect this path.
