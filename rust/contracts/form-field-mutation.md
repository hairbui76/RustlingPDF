...
and an optional JSON part named `names`. It must resolve to at least one
nonblank value. Accepted shapes are a string, an array of strings or
...
Existing appearance streams for remaining fields are preserved. Golden-corpus
coverage is still required for merged field/widget
...
HTTP tests cover nested and root field deletion, wrapper and
...
The JSON parser accepts a flat object;
...
commas, and coerces scalar values to strings. Blank or absent
data is an empty value map. Malformed JSON, unsupported scalar roots, and an
invalid `flatten` boolean return HTTP 400.

Field lookup uses the first exact fully qualified or partial name. Unknown and
blank keys are ignored. Text values support UTF-16 PDF strings. Checkboxes
accept common truthy aliases plus declared
...
Borders, backgrounds, alignment, comb fields, rich text, embedded/non-Latin
fonts, list-box highlighting, malformed appearances, missing widget pages, and
invalid geometry remain limited. RustlingPDF preserves the full UTF-16 `/V`,
but its current static appearance replaces characters outside WinAnsi with
`?`. PDFium is required for `flatten=true`.
...
with the fields `targetName`, `name`, `label`, `type`, `required`,
...
receive `_1`, `_2`, ... suffixes on collision. Same-type
...
Direct inline field dictionaries are currently skipped because in-place
replacement is only implemented for indirect terminal objects. Appearance
refresh behavior for unusual field dictionaries remains a golden-corpus
validation area.
