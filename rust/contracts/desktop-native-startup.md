...
- **Windows (MSI and NSIS)**: a newer version raises a dismissible banner.
  Nothing downloads until the user clicks "Update and restart"; the download
...
- **Linux deb/rpm**: the plugin rejects in-place updates there;
  ...
  dialog for it.
- **Linux AppImage (paused — "coming soon", v0.1.0 was the last one)**:
  while paused, the manifest carries no `linux-x86_64-appimage` entry, so
  the v0.0.8–v0.1.0 AppImage installs in the field get TargetNotFound from
  their check and go silent — an accepted consequence, documented in
  release.yml. When AppImage returns, its manifest entry returns with it.
...
release publishes. `latest.json` uses tauri-plugin-updater's per-installer
platform keys — `windows-x86_64-msi`, `windows-x86_64-nsis` — with the
minisign signatures inline. The running
app resolves the key matching its own baked-in bundle type, so an MSI
install only ever updates onto an MSI and an NSIS install onto NSIS; there
is deliberately no bare `{os}-{arch}` fallback key, which is what keeps
deb/rpm and portable runs out of in-place updating entirely. The signing key has
