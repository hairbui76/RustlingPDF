...
#[tokio::test]
async fn pdfa_route_validates_required_pdf_upload_before_starting_ghostscript()
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        post_multipart(
            Some(("source.txt", "application/pdf", b"not a PDF")),
            &[("outputFormat", "pdfa")],
        )
        .await?
        .status(),
...
#[tokio::test]
async fn pdfa_route_uses_java_profile_suffix_or_reports_missing_ghostscript()
-> Result<(), Box<dyn std::error::Error>> {
    let pdf = sample_pdf()?;
    let response = post_multipart(
        Some(("source.pdf", "application/pdf", &pdf)),
        &[("outputFormat", "pdfa-1"), ("strict", "false")],
    )
    .await?;
...
async fn post_multipart(
    ...
    let boundary = "stirling-pdfa-boundary";
