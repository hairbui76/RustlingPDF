...
#[tokio::test]
async fn automatic_redaction_validates_upload_patterns_and_padding()
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        post_multipart(
            Some(("source.pdf", "application/pdf", &source)),
            &[("listOfText", "Confidential"), ("customPadding", "-1")],
        )
        .await?
        .status(),
        StatusCode::BAD_REQUEST
...
#[tokio::test]
async fn automatic_redaction_removes_matching_text_or_reports_missing_pdfium()
-> Result<(), Box<dyn std::error::Error>> {
    let source = text_pdf()?;
...
async fn post_multipart(
    ...
    let boundary = "stirling-pdf-auto-redaction-boundary";
    ...
    Ok(app(1024 * 1024)
        .oneshot(
            Request::builder()
                .method("POST")
                .uri("/api/v1/security/auto-redact")
                .header(
                    header::CONTENT_TYPE,
