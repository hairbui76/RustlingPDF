...
#[tokio::test]
async fn pdf_to_xlsx_route_requires_an_upload() -> Result<(), Box<dyn std::error::Error>> {
    assert_eq!(
...
#[tokio::test]
async fn pdf_to_xlsx_route_extracts_a_workbook_or_reports_missing_pdfium()
-> Result<(), Box<dyn std::error::Error>> {
    ...
    if response.status() == StatusCode::NOT_IMPLEMENTED {
        assert!(std::env::var_os("STIRLING_PDFIUM_LIBRARY_PATH").is_none());
        return Ok(());
    }
    ...
    assert!(worksheet.contains(">A</t>"));
    assert!(worksheet.contains(">D</t>"));
    Ok(())
}
