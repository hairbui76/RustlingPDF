...
#[tokio::test]
async fn rejects_unsupported_and_unsafe_html_packages() -> Result<(), Box<dyn std::error::Error>> {
    assert_eq!(
        post_file(b"hello", "notes.txt").await?.status(),
        StatusCode::BAD_REQUEST
    );
    assert_eq!(
        post_file(&unsafe_zip()?, "package.zip").await?.status(),
        StatusCode::BAD_REQUEST
    );
    Ok(())
}
...
#[tokio::test]
async fn converts_html_or_reports_missing_weasyprint() -> Result<(), Box<dyn std::error::Error>> {
    let response = post_file(b"<h1>Safe title</h1><script>alert(1)</script>", "page.html").await?;
...
fn unsafe_zip() -> Result<Vec<u8>, Box<dyn std::error::Error>> {
