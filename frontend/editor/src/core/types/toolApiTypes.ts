...
// Generated by editor/scripts/generate-tool-api-types.mts from SwaggerDoc.json
...
export interface SignPDFWithCertRequest {
  ...
  password?: string;
  /**
   * Absolute path to the PKCS#11 driver library (required for PKCS11 type). Must be an allowed driver - a detected one or configured via RUSTLING_PKCS11_LIBRARIES.
   */
...
export interface TimestampPdfRequest {
  /**
   * URL of the RFC 3161 Time Stamp Authority (TSA) server. Must be one of the built-in presets (DigiCert, Sectigo, SSL.com, FreeTSA, MeSign) or a URL allowed in settings.yml (security.timestamp.customTsaUrls). If omitted, the server default is used.
   */
  tsaUrl?: string;
}
