...
SCRIPT_NAME="rustling-diagnostics"
DEFAULT_DAYS=1
...
log "=== RustlingPDF Diagnostics ==="
log "This tool collects a diagnostic bundle for RustlingPDF."
...
if [ "$MODE" = "1" ]; then
  ...
  if prompt_yes_no "Include /tmp/rustlingpdf?" "n"; then INCLUDE_TEMP=true; fi
...
if [ -f /etc/rustling_version ]; then
  write_kv "rustling_version_file" "$(tr -d '\r\n' < /etc/rustling_version)" "$SUMMARY"
fi
...
if [ "$INCLUDE_TEMP" = true ]; then
  log "Collecting /tmp/rustlingpdf..."
  copy_dir_filtered "/tmp/rustlingpdf" "$DATA_DIR" 0
fi
...
write_tree "/tmp/rustlingpdf" "${TREE_DIR}/tmp-rustlingpdf.txt"
